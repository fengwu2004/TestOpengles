...
void         
gles_25vec_layer_render
(T gvl, gles_25d_c_program_t gcp)
{

    gles_25d_c_program_vbo_draw_triangle(gcp, gvl->area_vbo_id, gvl->area_ver_count);
    
    glEnable(GL_PRIMITIVE_RESTART_FIXED_INDEX);
    gles_25d_c_program_draw_elements_vao(gcp, GL_LINE_LOOP, gvl->line_vao_id, gvl->line_ver_count + gvl->line_prim_reset_count);
    glDisable(GL_PRIMITIVE_RESTART_FIXED_INDEX);
}

int          
gles_25vec_layer_width
(T gvl)
{
    assert(gvl);
    return gvl->width;
